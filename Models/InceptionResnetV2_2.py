...
class InceptionResnet():
    ...
    def upsample(self, filters,size,strides=2,padding="same",batchnorm=False,dropout=0):
    ...
    def get_encoder(self, input_shape=[None,None,3],name="encoder", trainable = True): 
    ...
    def get_decoder(self,skips,dropout=0):
        up_stack = [
            self.upsample(512, 3,dropout=dropout),  # 4x4 -> 8x8
            self.upsample(256, 3,dropout=dropout),  # 8x8 -> 16x16
            self.upsample(128, 3,dropout=dropout),  # 16x16 -> 32x32
            self.upsample(64, 3,dropout=dropout),   # 32x32 -> 64x64
        ]
        x = skips[-1]
    ...
    def get_model(self,output_channels=1,size=224,name="InceptionResnetV2",dropout=0, trainable = True):
        x = inputs = tf.keras.layers.Input(shape=[size,size,3])

        skips = self.get_encoder(input_shape=list(x.shape[1:]), trainable=trainable)(x)

        x = self.get_decoder(skips, dropout=dropout)
